package net.jpct.client.util.gui;
 ...
 * The created texture is 2^n x 2^m as required by jPCT.
 * Useful for blitting purposes.
 * 
 * For an explanation of packing algorithm see:
...
public class TexturePack {
	 ...
	 * 
	 * @param imageType
	 *            must be one of the image types defined in
	 ...
	 * 
	 * @param useAlpha
	 *            will created texture use alpha ?
	 ...
	 * transparency or additional color. see
	 * {@link FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color)
	 ...
	 *         array instance
	 * @see FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int,
	 *      int, boolean, Color) FrameBuffer.blit(..)
	 */
	public Dimension blit(FrameBuffer buffer, int imageId, int destX, int destY, int transparency, boolean additive,
			Color color) {
		...
		return blit(buffer, entry, destX, destY, entry.bounds.width, entry.bounds.height, transparency, additive,
				color);
	}

	/**
	 * blits one of packed images completely with scaling or transparency or
	 * additional color see
	 * {@link FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color)
	 * FrameBuffer.blit(..)} for description of other parameters
	 * 
	 * @param buffer
	 ...
	 *      int, boolean, Color) FrameBuffer.blit(..)
	 */
	public Dimension blit(FrameBuffer buffer, int imageId, int destX, int destY, int destWidth, int destHeight,
		...
		Entry entry = entries.get(imageId);
		return blit(buffer, entry, destX, destY, destWidth, destHeight, transparency, additive, color);
	}
	...
	private Dimension blit(FrameBuffer buffer, Entry entry, int destX, int destY, int destWidth, int destHeight,
			int transparency, boolean additive, Color color) {
